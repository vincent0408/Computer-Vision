...
def get_tiny_images(img_paths):
    ...
    for ip in img_paths:
        ...
        if(w < h):
            cut_off = (h - w) // 2
            img = img[cut_off:h - cut_off]
        else:
            cut_off = (w - h) // 2
            img = img[:, cut_off:w - cut_off]
        small_img = cv2.resize(img, (16, 16), interpolation = cv2.INTER_AREA).flatten()
        small_img = cv2.normalize(small_img, None, 0, 1, cv2.NORM_MINMAX)
        tiny_img_feats.append(small_img)
    tiny_img_feats = np.squeeze(np.array(tiny_img_feats))
    #################################################################
    #                        END OF YOUR CODE                       #
    #################################################################
...
def build_vocabulary(img_paths, vocab_size):
    ...
    for ip in img_paths:
        img = cv2.imread(ip, 0).astype(np.float64)
        _, descriptors = dsift(img, step=[10, 10], fast=True)
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    test_predicts = []
    k = 5
    dist = cdist(test_img_feats, train_img_feats, metric='cityblock')
    sorted_k_index = np.argsort(dist, axis=1)[:,:k]
    k_canditates = np.array(train_labels)[sorted_k_index]

    for canditates in k_canditates:
        test_predicts.append(mode(canditates)[0][0])
    ###########################################################################
    #                               END OF YOUR CODE                          #
    ###########################################################################
